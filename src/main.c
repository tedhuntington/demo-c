...
enum mgos_app_init_result mgos_app_init(void) {
  char buf[8];
  int ret;
  uint16_t bmeaddr=0x77;
  uint16_t tsladdr=0x39;
...
//  LOG(LL_INFO, ("SDA=%d",i2c->cfg.sda_gpio));


    ret = mgos_i2c_read_reg_b(i2c,bmeaddr, 0xd0);
    LOG(LL_INFO, ("BME Register value is 0x%x", ret));  
  
  
    ret = mgos_i2c_read_reg_b(i2c,tsladdr, 0x0a);
    LOG(LL_INFO, ("TSL Register value is 0x%x", ret));    
